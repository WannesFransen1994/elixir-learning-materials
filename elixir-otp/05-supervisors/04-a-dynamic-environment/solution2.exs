defmodule Employee do
  ...
  def child_spec({id, colleagues}) do
    %{ id: id, start: { Employee, :start_link, [{id, colleagues}] } }
  end
end

DynamicSupervisor.start_link(name: EmployeeSupervisor, strategy: :one_for_one)

employees = [A, B, C]

Enum.each( employees, fn id ->
  colleagues = Enum.reject(employees, &(&1 == id))
  DynamicSupervisor.start_child(EmployeeSupervisor, { Employee, {id, colleagues} })
end )
