...
_Note: All the following modules are prefixed with the project name. In this case, that's `ExerciseSolution`_
...
```elixir
iex> GameServer.add_instance InstanceA
:ok
iex> GameServer.add_instance InstanceB
:ok
iex> GameServer.add_instance InstanceC
:ok
iex> GameServer.list_instances
...
```elixir
iex> InstanceSupervisor.add_instance(name: InstanceA)
{:ok, #PID<0.167.0>}
iex> InstanceSupervisor.add_instance(name: InstanceA)
{:error, {:already_started, #PID<0.167.0>}}
iex> InstanceSupervisor.add_instance([])
{:error, :no_name_param}
iex> InstanceSupervisor.add_instance(name: InstanceB)
{:ok, #PID<0.174.0>}
```
